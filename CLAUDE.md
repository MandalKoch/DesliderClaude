...
- **Admin console at `/admin`** (role-gated). Username → role is config-driven: `Admin:Usernames` in `appsettings.json` (or `Admin__Usernames__0=foo` env var). Admins see a red **Admin** nav button, reach a single-page control deck with: site-wide stats (users, visitors, nights, open/closed, voters, swipes, swipes in 24 h), top games aggregated by name, a nights table with delete, and a users table with delete (self-deletion blocked).
